...
if (is_admin()) {
  ...
  if (!class_exists("wp_server_stats")) {
    ...
    class wp_server_stats
    {
      ...
      public function wpss_cache_purge_callback()
      {
        ...
        echo 'WP Server Stats Cache has been purged successfully!';
        ...
        if ($server_location === false) {
          ...
          if ($this->validate_ip_address($ip)) {
            if ($this->use_ipapi_pro == 'Yes' && !empty($ipapi_pro_key)) { // Use the pro version of IP-API query
              $response = wp_safe_remote_get("https://pro.ip-api.com/json/{$ip}?key={$ipapi_pro_key}");
            ...
            $ip_details = wp_remote_retrieve_body($response);
            $ip_details = json_decode($ip_details, true);

            if (!empty($ip_details) && $ip_details['status'] === 'success') {
              $server_location = sanitize_text_field( $ip_details['city'] . ', ' . $ip_details['country'] );
              set_transient('wpss_server_location', $server_location, WEEK_IN_SECONDS);
            } else {
              if (empty($ip_details['message'])) {
                if ($this->use_ipapi_pro == 'Yes') {
                  $server_location = 'You\'ve provided a wrong IP-API Pro Key';
                } else {
                  $server_location = sanitize_text_field( $ip_details['status'] );
                }
              } else {
                $server_location = sanitize_text_field( $ip_details['message'] );
              }
            }
          } else {
      ...
      **/
      ...
      public function dashboard_output()
      {
        ...
        ?>
            <ul>
              <li><strong><?php esc_html_e('Server OS', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->server_os()); ?>&nbsp;/&nbsp;<?php echo esc_html(PHP_INT_SIZE * 8) . esc_html__('Bit OS', 'wp-server-stats'); ?></span></li>
              <li><strong><?php esc_html_e('Server Software', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($_SERVER['SERVER_SOFTWARE']); ?></span></li>
              <li><strong><?php esc_html_e('Server IP', 'wp-server-stats'); ?></strong> : <span><?php echo ($this->validate_ip_address($this->check_server_ip()) ? sanitize_text_field($this->check_server_ip()) : 'ERROR IP096T'); ?></span></li>	
              <li><strong><?php esc_html_e('Server Port', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($_SERVER['SERVER_PORT']); ?></span></li>
              <li><strong><?php esc_html_e('Server Location', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->check_server_location()); ?></span></li>
              <li><strong><?php esc_html_e('Server Hostname', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field(gethostname()); ?></span></li>
              <li><strong><?php esc_html_e('Site\'s Document Root', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($_SERVER['DOCUMENT_ROOT'] . '/'); ?></span></li>
              <li><strong><?php esc_html_e('Memcached Enabled', 'wp-server-stats'); ?></strong> : <span><?php echo (class_exists('Memcache') ? esc_html__('Yes', 'wp-sever-stats') : esc_html__('No', 'wp-server-stats')); ?></span></li>
              <?php if ($this->isShellEnabled()) : ?>
              <li><strong><?php esc_html_e('Total CPUs', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->check_cpu_count()) . ' / ' . sanitize_text_field($this->check_core_count()) . esc_html__('Cores', 'wp-server-stats'); ?></span></li>
              <li><strong><?php esc_html_e('Total RAM', 'wp-server-stats'); ?></strong> : <span><?php echo (is_numeric($this->check_total_ram()) ? sanitize_text_field($this->format_filesize_kB($this->check_total_ram())) : sanitize_text_field($this->check_total_ram())); ?></span></li>
              <li><strong><?php esc_html_e('Real Time Free RAM', 'wp-server-stats'); ?></strong> : <span id="realtime_free_ram"></span></li>
              <li><strong><?php esc_html_e('Real Time RAM Usage', 'wp-server-stats'); ?></strong> : <span id="realtime_ram_usage"></span></li>
              <?php endif; ?>
            <ul>
            <?php if ($this->isShellEnabled()) : ?>
            ...
            <span style="line-height: 2.5em;"><strong><?php esc_html_e('Real Time CPU Load', 'wp-server-stats') ?>:</strong></span>
            <div class="progressbar">
              <div style="border:1px solid #DDDDDD; background-color:#F9F9F9;	border-color: rgb(223, 223, 223); box-shadow: 0px 1px 0px rgb(255, 255, 255) inset; border-radius: 3px;">
            ...
            <?php if (class_exists('Memcache')) : ?>
              <div class="wpss_show_buttons content-center">
                <a href="<?php echo esc_url(get_admin_url()); ?>admin.php?page=wpss_memcache_info" title="Checkout Memcached Info" class="wpss_btn button button-small"><?php esc_html_e('Check More Memcached Info', 'wp-server-stats'); ?></a>
              </div>
            <?php endif; ?>
            <hr />
            <ul>
              <li><strong><?php esc_html_e('Database Software', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->database_software()); ?></span></li>
              <li><strong><?php esc_html_e('Database Version', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->database_version()); ?></span></li>
              <li><strong><?php esc_html_e('Maximum No. of Connections', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->database_max_no_connection()); ?></span></li>
              <li><strong><?php esc_html_e('Maximum Packet Size', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->database_max_packet_size()); ?></span></li>
              <li><strong><?php esc_html_e('Database Disk Usage', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->database_disk_usage()); ?></span></li>
              <li><strong><?php esc_html_e('Index Disk Usage', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->index_disk_usage()); ?></span></li>
            </ul>
            <div class="wpss_show_buttons content-center">
              <a href="<?php echo esc_url(get_admin_url()); ?>admin.php?page=wpss_sql_info" title="Checkout More Database Info" class="wpss_btn button button-small"><?php esc_html_e('Check More Database Info', 'wp-server-stats'); ?></a>
            </div>
            <hr />
            <ul>
              <li><strong><?php esc_html_e('PHP Version', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field(PHP_VERSION); ?></span></li>
              <li><strong><?php esc_html_e('PHP Max Upload Size', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->php_max_upload_size()); ?></span></li>
              <li><strong><?php esc_html_e('PHP Max Post Size', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->php_max_post_size()); ?></span></li>
              <li><strong><?php esc_html_e('PHP Max Execution Time', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->php_max_execution_time()) . ' ' . esc_html__("sec", "wp-server-stats"); ?></span></li>
              <li><strong><?php esc_html_e('PHP Short Tag', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->php_short_tag()); ?></span></li>
              <li><strong><?php esc_html_e('PHP Memory Limit', 'wp-server-stats'); ?></strong> : <span><?php echo sanitize_text_field($this->check_limit()); ?></span></li>
              <li><strong><?php esc_html_e('Real Time PHP Memory Usage', 'wp-server-stats'); ?></strong> : <span id="mem_usage_mb"></span></li>
            </ul>
            <div class="progressbar">
              <div style="border:1px solid #DDDDDD; background-color:#F9F9F9;	border-color: rgb(223, 223, 223); box-shadow: 0px 1px 0px rgb(255, 255, 255) inset; border-radius: 3px;">
            ...
            <div class="wpss_show_buttons content-center">
              <a href="<?php echo esc_url(get_admin_url()); ?>admin.php?page=wpss_php_info" title="Checkout More PHP Info" class="wpss_btn button button-small"><?php esc_html_e('Check More PHP Info', 'wp-server-stats'); ?></a>
            </div>
            <?php if ($this->isShellEnabled()) : ?>
            <hr style="margin-top: 15px; margin-bottom: 0px;" />
            <span style="line-height: 2.5em; margin-left: auto; margin-right: auto; display: table;"><strong><?php esc_html_e('Server Uptime', 'wp-server-stats') ?></strong></span>
            <div style="margin-top: 20px;">
              <div class="uptime" style="font-size: 20px;"></div>
            </div>
        <?php else : ?>
              <hr style="margin-top: 15px; margin-bottom: 15px;" />
              <p style="text-align: justify;"><strong><?php esc_html_e('Special Note', 'wp-server-stats'); ?>:</strong> <?php echo wp_kses('Hi, please note that PHP 
              ...
              please ask your host to enable <code>shell_exec()</code> function and give it executable permission.', wp_kses_allowed_html()); ?></p>
            <?php endif;
          endif;
        }
      ...
      /* Function that will show the indepth PHP information */
        ...
        <div class="wrap wpss_info">
          <h1><?php esc_html_e('PHP Information - WP Server Stats', 'wp-server-stats'); ?></h1>
          <h3><?php esc_html_e('This page will show you the in-depth information about the PHP installasion on your server', 'wp-server-stats'); ?></h3>
          <hr />
          <?php
    ...
    public function sql_details()
    {
      ?>
        <div class="wrap wpss_info">
          <h1><?php esc_html_e('Database Information - WP Server Stats', 'wp-server-stats'); ?></h1>
          <h3><?php esc_html_e('This page will show you the in-depth information about your database', 'wp-server-stats'); ?></h3>
          <hr />
          <h2><?php esc_html_e('Basic Database Information', 'wp-server-stats'); ?></h2>
          <table class="widefat">
            <thead>
              <tr>
                <th><?php esc_html_e('Variable Name', 'wp-server-stats'); ?></th>
                <th><?php esc_html_e('Value', 'wp-server-stats'); ?></th>
              </tr>
            </thead>
            <tfoot>
              <tr>
                <td class="e"><?php esc_html_e('Variable Name', 'wp-server-stats'); ?></td>
                <td><?php esc_html_e('Value', 'wp-server-stats'); ?></td>
              </tr>
            </tfoot>
            <tbody>
              <tr>
                <td class="e"><?php esc_html_e('Database Software', 'wp-server-stats'); ?></td>
                <td class="v"><?php echo sanitize_text_field($this->database_software()); ?></td>
              </tr>
              <tr>
                <td class="e"><?php esc_html_e('Database Version', 'wp-server-stats'); ?></td>
                <td class="v"><?php echo sanitize_text_field($this->database_version()); ?></td>
              </tr>
              <tr>
                <td class="e"><?php esc_html_e('Maximum No. of Connections', 'wp-server-stats'); ?></td>
                <td class="v"><?php echo sanitize_text_field($this->database_max_no_connection()); ?></td>
              </tr>
              <tr>
                <td class="e"><?php esc_html_e('Maximum Packet Size', 'wp-server-stats'); ?></td>
                <td class="v"><?php echo sanitize_text_field($this->database_max_packet_size()); ?></td>
              </tr>
              <tr>
                <td class="e"><?php esc_html_e('Database Disk Usage', 'wp-server-stats'); ?></td>
                <td class="v"><?php echo sanitize_text_field($this->database_disk_usage()); ?></td>
              </tr>
              <tr>
                <td class="e"><?php esc_html_e('Index Disk Usage', 'wp-server-stats'); ?></td>
                <td class="v"><?php echo sanitize_text_field($this->index_disk_usage()); ?></td>
              </tr>
            </tbody>
          </table>
          <div class="clear give-some-space"></div>
          <hr />
          <h2><?php esc_html_e('Advanced Database Information', 'wp-server-stats'); ?></h2>
          <table class="widefat">
            <thead>
              <tr>
                <th><?php esc_html_e('Variable Name', 'wp-server-stats'); ?></th>
            ...
            <tfoot>
              <tr>
                <td><?php esc_html_e('Variable Name', 'wp-server-stats'); ?></td>
                <td><?php esc_html_e('Value', 'wp-server-stats'); ?></td>
              </tr>
            </tfoot>
            <tbody>
            <?php
              ...
              if (!empty($dbinfo)) {
                foreach ($dbinfo as $info) {
                  echo '<tr><td class="e">' . sanitize_text_field($info->Variable_name) . '</td><td class="v">' . wp_kses(htmlspecialchars($info->Value), wp_kses_allowed_html()) . '</td></tr>';
                }
              } else {
                echo '<tr><td>' . esc_html__('Something went wrong!', 'wp-server-stats') . '</td><td>' . esc_html__('Something went wrong!', 'wp-server-stats') . '</td></tr>';
              }
            ?>
            </tbody>
          </table>
    ...
    public function memcache_details()
    {
      if (class_exists('Memcache')) {
        ...
        ?>
          <div class="wrap wpss_info">
            <h1><?php esc_html_e('Memcached Information - WP Server Stats', 'wp-server-stats'); ?></h1>
            <h3><?php esc_html_e('This page will show you the in-depth information about your memcache server', 'wp-server-stats'); ?></h3>
            <hr />
            <table class="widefat">
              <thead>
                <tr>
                  <th><?php esc_html_e('Variable Name', 'wp-server-stats'); ?></th>
                  <th><?php esc_html_e('Value', 'wp-server-stats'); ?></th>
                  <th><?php esc_html_e('Description', 'wp-server-stats'); ?></th>
                </tr>
              </thead>
              <tfoot>
                <tr>
                  <td><?php esc_html_e('Variable Name', 'wp-server-stats'); ?></td>
                  <td><?php esc_html_e('Value', 'wp-server-stats'); ?></td>
                  <td><?php esc_html_e('Description', 'wp-server-stats'); ?></td>
                </tr>
              </tfoot>
              <tbody>
                <tr>
                  <td class="e"><?php esc_html_e('pid', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field($memcachedinfo['pid']); ?></td>
                  <td class="v"><?php esc_html_e('Process ID', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('vptime', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field($uptime); ?></td>
                  <td class="v"><?php esc_html_e('Number of days since the process was started', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('version', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field($memcachedinfo['version']); ?></td>
                  <td class="v"><?php esc_html_e('Memcached Version', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('rusage_user', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field($memcachedinfo['rusage_user']); ?></td>
                  <td class="v"><?php esc_html_e('Number of seconds the cpu has devoted to the process as the user', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('rusage_system', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field($memcachedinfo['rusage_system']); ?></td>
                  <td class="v"><?php esc_html_e('Number of seconds the cpu has devoted to the process as the system', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('curr_items', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['curr_items'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of items currently in memcached', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('total_items', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['total_items'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of items that have passed through memcached', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('bytes', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field($this->format_filesize($memcachedinfo['bytes'])); ?></td>
                  <td class="v"><?php esc_html_e('Memory size currently used by <code>curr_items</code>', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('limit_maxbytes', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field($this->format_filesize($memcachedinfo['limit_maxbytes'])); ?></td>
                  <td class="v"><?php esc_html_e('Maximum memory size allocated to memcached', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('curr_connections', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['curr_connections'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of open connections to memcached', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('total_connections', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['total_connections'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of connections opened since memcached started running', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('connection_structures', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['connection_structures'])); ?></td>
                  <td class="v"><?php esc_html_e('Number of connection structures allocated by the server', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('cmd_get', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['cmd_get'])); ?></td>
                  <td class="v"><?php esc_html_e('Total GET commands issued to the server', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('cmd_set', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['cmd_set'])); ?></td>
                  <td class="v"><?php esc_html_e('Total SET commands issued to the server', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('cmd_flush', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['cmd_flush'])); ?></td>
                  <td class="v"><?php esc_html_e('Total FLUSH commands issued to the server', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('get_hits', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['get_hits']) . '(' . $cache_hit . '%)'); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a GET command was <strong>able</strong> to retrieve and return data', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('get_misses', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['get_misses']) . '(' . $cache_miss . '%)'); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a GET command was <strong>unable</strong> to retrieve and return data', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('delete_hits', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['delete_hits'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a DELETE command was <strong>able</strong> to delete data', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('delete_misses', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['delete_misses'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a DELETE command was <strong>unable</strong> to delete data', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('incr_hits', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['incr_hits'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a INCR command was <strong>able</strong> to increment a value', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('incr_misses', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['incr_misses'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a INCR command was <strong>unable</strong> to increment a value', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('decr_hits', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['decr_hits'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a DECR command was <strong>able</strong> to decrement a value', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('decr_misses', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['decr_misses'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a DECR command was <strong>unable</strong> to decrement a value', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('cas_hits', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['cas_hits'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a CAS command was <strong>able</strong> to compare and swap data', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('cas_misses', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['cas_misses'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of times a CAS command was <strong>unable</strong> to compare and swap data', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('cas_badval', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['cas_badval'])); ?></td>
                  <td class="v"><?php esc_html_e('The "cas" command is some kind of Memcached\'s way to avoid locking. "cas" calls with bad identifier are counted in this stats key', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('bytes_read', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field($this->format_filesize($memcachedinfo['bytes_read'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of bytes input into the server', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('bytes_written', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field($this->format_filesize($memcachedinfo['bytes_written'])); ?></td>
                  <td class="v"><?php esc_html_e('Total number of bytes written by the server', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('evictions', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['evictions'])); ?></td>
                  <td class="v"><?php esc_html_e('Number of valid items removed from cache to free memory for new items', 'wp-server-stats'); ?></td>
                </tr>
                <tr>
                  <td class="e"><?php esc_html_e('reclaimed', 'wp-server-stats'); ?></td>
                  <td class="v"><?php echo sanitize_text_field(number_format_i18n($memcachedinfo['reclaimed'])); ?></td>
                  <td class="v"><?php esc_html_e('Number of items reclaimed', 'wp-server-stats'); ?></td>
                </tr>
              </tbody>
            </table>
          </div>
  ...
  public function admin_page_design()
  {
    ...
    ?>
      <div class="wrap">
        <h1><?php esc_html_e('WP Server Stats Settings', 'wp-server-stats'); ?></h1>
        <h3><?php esc_html_e('On this page you will be able to change some critical settings of WP Server Stats', 'wp-server-stats'); ?></h3>
        <h4><?php esc_html_e('Please note the below form uses HTML5, so, make sure you are using any of the HTML5 compliance browsers like IE v11+, Microsoft Edge, Chrome v49+, Firefix v47+, Safari v9.1+, Opera v39+', 'wp-server-stats'); ?></h4>
        <hr />
        <div id="wpss-main">
    ...
    public function use_ipapi_pro()
    {
      ...
      ?>
        ...
        <span><?php esc_html_e('Yes', 'wp-server-stats'); ?></span>
        ...
        <span><?php esc_html_e('No', 'wp-server-stats'); ?></span>
        <br />
        <p><?php printf(
              'By default this plugin uses the free API from %1$sIP-API.com%2$s which allows %3$s150 requests/min%4$s. But for high traffic websites, this might be very small and may generate %3$s503 Error%4$s if you try to do more than %3$s150 req/min%4$s. To resolve this problem, you can use the %5$sPaid Version of IP-API%6$s and provide your paid key below which will allow you to do %7$sUnlimited%8$s nuber of requests.',
    ...
    public function ipapi_pro_key()
    {
      ...
      ?>
        ...
        <p><?php printf(
              'Please provide your paid API key of ip-api.com which you have %1$sreceived over email%2$s after %3$spurchasing the paid IP-API subscription%4$s. %5$sCheck this screenshot%6$s to understand what key I\'m talking about.',
              '<strong>',
              '</strong>',
    ...
    public function validate_options($fields)
    {
      ...
      $valid_fields['refresh_interval'] = sanitize_text_field($refresh_interval);

      $memcache_host = trim($fields['memcache_host']);
      $valid_fields['memcache_host'] = sanitize_text_field($memcache_host);

      $memcache_port = trim($fields['memcache_port']);
      $valid_fields['memcache_port'] = sanitize_text_field($memcache_port);

      $valid_fields['use_ipapi_pro'] = sanitize_text_field(trim($fields['use_ipapi_pro']));
      ...
      foreach ($fields as $key => $value) {
        if (preg_match('/_color/', $key)) {
          ...
          $color[$key] = sanitize_text_field($color[$key]);
    ...
    public function show_admin_notice()
    {
      ...
      if (empty($welcome_notice_curr_state)) {
        printf('<div id="wpss_donate_notice" class="%1$s"><p>%2$s</p></div>', esc_attr($class), wp_kses($message, wp_kses_allowed_html()));
        echo "<script>
            (function($){
            $('#wpss_donate_notice').on('click', '.notice-dismiss', function(){
              jQuery.ajax({
                ...
                data: {
                  ...
                  \"nonce\": \"" . esc_attr(wp_create_nonce("wpss_wn_nonce")) . "\"
                },
